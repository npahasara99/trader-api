...
## Multi-timeframe execution scenarios

- `app/chart_context.py` deterministically summarizes daily, 1-hour, and
  30-minute OHLCV. Every timeframe reports availability and bar count.
- `app/scenario_engine.py` creates separate enter-now, pullback, breakout, and
  repair alternatives from support/resistance, ATR, ranges, and moving
  structure. Numeric levels are never generated by the LLM layer.
- `preferred_scenario` and `execution_action` add execution detail without
  removing backward-compatible `BUY` / `WAIT` / `AVOID` output.
- Intraday bars are fetched only for fully planned candidates and cached by
  ticker/timeframe/lookback. Missing intraday data does not fail the plan.
- TradingView monitoring events are persistence-only. They request structured
  re-evaluation and cannot call the broker execution path.
